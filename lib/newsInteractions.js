...
export async function fetchNewsByCategoryFromDB(category) {
    try {
        const news = await fetch(`/api/News/category?category=${encodeURIComponent(category)}`, { method: 'GET' });
...
export async function fetchOneNewsFromDB(id) {
    try{
        const news = await fetch(`/api/News/specific?id=${encodeURIComponent(id)}`, { method: 'GET' });
        const data = await news.json();
