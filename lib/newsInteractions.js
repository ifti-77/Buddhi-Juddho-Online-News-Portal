...
const API_BASE_URL = `${process.env.NEXT_PUBLIC_APP_URL}/api/News/`;
...
export async function fetchOnlyLatestNewsFromDB() {
    try {
        const news = await fetch(`${API_BASE_URL}latest`, { method: 'GET' });
        const data = await news.json();
        if(!news.ok){
            return { newsItems: [], count: 0, error: data.error };
        }
